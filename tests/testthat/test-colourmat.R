...
test_that ("colours", {
    ...
    expect_error (
        colour_mat (cols = letters [1:4]),
        "Invalid colours: a"
    )
    expect_error (
        colour_mat (cols = c ("red", "blue", letters [1:4])),
        "Invalid colours: a"
    )
    expect_error (
        colour_mat (cols = rep (NA, 4)),
...
test_that ("n", {
    ...
    expect_error (
        colour_mat (cols = 1:4, n = c (2, "a")),
        "n must be numeric"
    )
    expect_error (
        colour_mat (cols = 1:4, n = c (2, NA)),
        "n can not be NA"
    )
})
...
test_that ("rotate", {
    expect_error (
        colour_mat (cols = 1:4, rotate = "a"),
        "rotate must be numeric"
    )
    expect_error (
        colour_mat (cols = 1:4, rotate = NA),
        "rotate must be numeric"
    )
    expect_warning (
        colour_mat (cols = 1:4, rotate = 1:2),
        "rotate has length > 1"
    )
})
