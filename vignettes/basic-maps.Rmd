...
png (file='map_a1.png', height=mapht, width=mapwd)
print (map)
graphics.off ()
```
![](map_a1.png)

Note that `osmplotr` maps contain no margins and fill the entire plot area.
...
map <- add_osm_objects (map, dat_B, col='gray40')
```
The `ggplot2` object is scaled to the relative proportions of the bounding box,
and so may be viewed simply by calling `map` or
```{r, eval=FALSE}
print (map)
```
```{r map2-print, echo=FALSE}
mapht <- mapwd * diff (bbox_small [2,]) / diff (bbox_small [1,])
png (file='map_a2.png', height=mapht, width=mapwd)
print (map)
graphics.off ()
```
![](map_a2.png)
...
map <- plot_osm_basemap (bbox=bbox_small, bg='gray20')
map <- add_osm_objects (map, dat_B, col='gray40', border='orange', size=0.2)
png (file='map_a3.png', height=mapht, width=mapwd)
print (map)
graphics.off ()
```
![](map_a3.png)
...
map <- add_osm_objects (map, dat_H, col='gray70', size=0.7)
map <- add_osm_objects (map, dat_T, col='green', size=2, shape=1)
png (file='map_a4.png', height=mapht, width=mapwd)
print (map)
graphics.off ()
```
![](map_a4.png)
...
list of possible devices).  Printing to a file requires manual specification of
device dimensions:
```{r, eval=FALSE}
mapwd <- 10
mapht <- mapwd * diff (bbox_small [2,]) / diff (bbox_small [1,])
```
and then using either `ggsave` 
```{r ggsave, eval=FALSE}
ggplot2::ggsave (filename='map.png', plot=map, width=mapwd, height=mapht, units='cm')
```
or by explicitly calling a desired device:
```{r pngsave, eval=FALSE}
png (width=mapwd, height=mapht, units='cm', res=72, file='map.png')
print (map)
graphics.off ()
```
...
print (map)
```
```{r map8-bbox, echo=FALSE}
bbox <- as.vector (slot (dat_B, 'bbox'))
bbox <- rbind (bbox, as.vector (slot (dat_H, 'bbox')))
bbox <- rbind (bbox, as.vector (slot (london$dat_P, 'bbox')))
bbox <- rbind (bbox, as.vector (slot (london$dat_A, 'bbox')))
bbox <- rbind (bbox, as.vector (slot (london$dat_G, 'bbox')))
bbox <- rbind (bbox, as.vector (slot (london$dat_T, 'bbox')))
bbox <- osmplotr::get_bbox (c (min (bbox [,1]), min (bbox [,2]),
...
map <- dat$map
png (file='map_a8.png', height=mapht, width=mapwd)
print (map)
graphics.off ()
```
![](map_a8.png)
...
As described above 
([2.4 Downloading with `osm_structures` and `make_osm_map`](#2.4 downloading2)),
if existing `osm_data` are not passed to `make_osm_map`, then all data will be
downloading by that function and returned in the `$osm_data` list component.
