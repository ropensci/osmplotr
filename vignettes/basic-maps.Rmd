...
customisable maps. Data are downloaded via the [`osmdata`
package](https://cran.r-project.org/package=osmdata), and different aspects of
map data - such as roads, buildings, parks, or water bodies - are able to be
visually customised.  This vignette demonstrates both data downloading and the
creation of simple maps. The subsequent vignette
...
`osmplotr`. The `osmplotr` functionn `extract_osm_objects` is effecitvely just a
...
1. `osmdata` returns *all* spatial data for a given query; that is, *all*
   ...
   returns a single specifed geometric type.
...
As demonstrated abovve, negation can be specified by pre-pending `!` to the
...
may be considerably larger than the desired boundaries, partiuclarly because
