...
nrow (highways3)
```
`connect_highways` returns a matrix of coordinates representing the shortest
...
the following code (in which both `dat_H` and `dat_HP` are `sf` data frames of
class `sfc_LINESTRING`:
```{r, eval = TRUE}
osm_basemap (bbox = bbox, bg = "gray20") %>%
...
colour schemes: one for all line objects, and a potentially different one for
all polygon objects.
...
points, as illustrated in the next example, which overlays trees on the
map.
