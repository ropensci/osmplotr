...
```{r, eval=FALSE}
dat_B <- extract_osm_objects (key="building", bbox=bbox)$dat
```
...
```{r, eval=FALSE}
dat_B <- extract_osm_objects (key="building", bbox=bbox)$dat
dat_H <- extract_osm_objects (key="highway", bbox=bbox)$dat
dat_T <- extract_osm_objects (key="natural", value="tree", bbox=bbox)$dat
```
...
dat_BR <- extract_osm_objects (key="building", value="residential",
                               bbox=bbox)$dat
dat_HP <- extract_osm_objects (key="highway", value="primary", bbox=bbox)$dat
```
```{r, echo=FALSE}
...
different colour.
```{r, eval=FALSE}
dat_H <- extract_osm_objects (key="highway", value="!primary", bbox=bbox)$dat
```
```{r, echo=FALSE}
dat_H <- london$dat_H
```
