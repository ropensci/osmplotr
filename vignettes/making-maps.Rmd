...
can be made using the following simple steps:
```{r, echo=FALSE}
...
```
The `extract_osm_objects` function returns spatial (`sp`) `data.frame` objects
of an appropriate types.
```{r}
class (dat_B)
class (dat_H)
class (dat_T)
...
plot, which may be scaled in proportion to the latitudinal and longitudinal
range of the objects to be plotted using `get_xylims`, or any other desired
limits may be passed. Objects may then be overlaid on this basemap using
`add_osm_objects`.
```{r}
...
`osmar`, preventing rivers from being included in plots. This problem will be
addressed in future releases of `osmplotr`.
...
## 5 Bounding areas within named highways
...
cols_B <- RColorBrewer::brewer.pal (4, "Set2") [2:4] # first colour is green
...
### 5.1 The highways2polygon function
...
the initial basemap with the following lines.
```{r, eval=TRUE}
structures <- c ("amenity", "grass", "park", "natural")
structs <- osm_structures (structures=structures, col_scheme="dark")
plot_osm_basemap (xylims=get_xylims (bbox), file="map20.png")
#plot_osm_basemap (xylims=get_xylims (bbox))
for (i in seq (nrow (structs) - 1)) # because background is last row
{
    j <- match (paste ("dat_", structs$suffix [i], sep=""),
                names (london))
    add_osm_objects (london [[j]], col=structs$cols [i])
}
graphics.off ()
```
![map](map20.png)

And then the desired areas highlighted using colours from `RColorBrewer`.
Highway colours are darkened versions of the `RColorBrewer` colours used for
buildings.
```{r, message=FALSE}
require (RColorBrewer)
```
```{r}
cols <- brewer.pal (4, "Set1") [2:4] # first colour is green
# darken colours for highways by first converting to rgb
cols_rgb <- 0.6 * col2rgb (cols)
# convert to hex using `sprintf ('%X',...)`
cols_hex <- rep (NA, ncol (cols_rgb))
for (i in seq (ncol (cols_rgb)))
{
    s <- sprintf ('%X', round (cols_rgb [,i]))
    cols_hex [i] <- paste0 ("#", s [1], s [2], s [3])
}
cols
cols_hex
```
...
group_osm_objects (london$dat_H, groups=groups, boundary=0,
                   col_extra=col_extra_H, colmat=FALSE, col=cols_hex)
group_osm_objects (london$dat_HP, groups=groups, boundary=0,
                   col_extra=col_extra_H, colmat=FALSE, col=cols_hex)
graphics.off ()
```
...
demonstrated by the above error message. There are nevertheless steps that can
be taken to overcome potential problems, as now explored in the final section of
this vignette.

### 5.2 highways2polygon in detail

`highways2polygon` finds a sequential polygon that circularly connected the
...
warning message. The first step is ordering the OSM data sequentially, with this
first done for the individual highways using `order_lines`. As explained in
`?order_lines`, this function:

> Accepts a SpatialLinesDataFrame representing an OpenStreetMap line object such
> as a highway. The list items of these objects are arbitrarily organised within
> OpenStreetMap. This function orders the components, returning a list of
> components each of which is ordered sequentially along the line.  Points of
> intersection between components are also inserted where these are not
> explicitly present in OpenStreetMap. Nodes are also sequentially renumbered,
> starting at (i0+1)
