...
The above `structs` can be used to make a map using the `london` data:
```{r, eval=FALSE}
make_osm_map (osm_data=london, structures=structs, dat_prefix="dat_",
              file="map7.png")
graphics.off ()
```
```{r, echo=FALSE}
# knitr evidently does not include figures that are generated entire within
# external function calls, so this just repeats the relevant lines
...
plot_osm_basemap (xylims=xylims, bg=bg, file="map7.png")
for (i in seq (nrow (structures) - 1))
{
    obji <- paste0 ("dat_", structures$suffixes [i])
    add_osm_objects (london [[obji]], col=structures$cols [i])
}
graphics.off()
```
![map](map7.png)
...
```{r, eval=FALSE}
make_osm_map (osm_data=london, structures=structs, dat_prefix="dat_",
              bbox=bbox, file="map8.png")
graphics.off ()
```
```{r, echo=FALSE}
plot_osm_basemap (xylims=get_xylims (bbox), bg=bg, file="map8.png")
for (i in seq (nrow (structures) - 1))
{
    obji <- paste0 ("dat_", structures$suffixes [i])
    add_osm_objects (london [[obji]], col=structures$cols [i])
...
`colour_mat` may be illustrated by plotting inclusive groups defined by simple
points.
```{r}
x <- xylims$xrange [1] + rep (0.5 + 0:3, each=3) * diff (xylims$xrange) / 4
y <- xylims$yrange [1] + rep (0.5 + 0:2, times=4) * diff (xylims$yrange) / 3
pts <- cbind (x, y)
pts <- apply (pts, 1, function (i) 
...
group_osm_objects (dat_B, groups=pts, col_extra=NA, 
                   colmat=TRUE, boundary=-1)
graphics.off ()
```
![map](map14.png)
