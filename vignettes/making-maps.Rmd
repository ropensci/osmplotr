...
library (osmplotr)
library (maptools) # Also needed for this vignette
```
...
The function `osm_structures` is an aid both for downloading of multiple OSM
data types, and for plotting (as described below). This function returns a
`data.frame` of OSM structure types, associated `key-value` pairs, unique
suffices which may be appended to data structures for storage purposes, and
suggested colours.
...
structure names.  A list of desired structures can also be passed to this
...
#### 2.3.1 The `london` data of `osmplotr`
...
### 2.4 Downloading connected highways
...
achieves this is `connect_highways`, examples of which are,
```{r, eval=FALSE}
...
The extraction of bounding polygons from named highways is not failsafe, and
indeed the first of the above calls (for `highways1`) generates a warning
message that
```{r, echo=FALSE}
...
is useful to examine `connect_highways` in more detail.

#### 2.4.1 `connect_highways` in detail
...
specified background colour. Portions of maps can also be printed by creating a
...
png (height=mapht, width=mapwd, file="map3.png")
print (map)
graphics.off ()
```
![map3](map3.png)
...
graphics.off ()
```
The first two lines scale the proportions of the graphics device to the bounding
box.


### 3.1 Plotting different OSM Structures

As demonstrated above, OSM structures are identified either with a single `key`,
in which case all objects matching that key will be returned regardless of
`value`, or a `key-value` pair. `key-value` pairs are particularly useful for
contrasting different kinds of object, for example primary and non-primary
highways:
...
```
Note that the `size` argument is passed to the respective `ggplot` function and
so determines the width of lines (for polygons or lines) and the sizes of
points.
```{r, eval=FALSE}
print (map)
```
```{r map5-print, echo=FALSE}
png (height=mapht, width=mapwd, file="map5.png")
print (map)
graphics.off ()
```
![map5](map5.png)
...
The additional `key-value` pairs demonstrated above can be used to extract very
specific objects.
```{r, eval=FALSE}
extra_pairs <- c ("name", "Royal.Festival.Hall")
...
```
These objects can then be indiviually plotted with different colour schemes.
```{r map7}
...
`make_osm_map` is `osm_data`. If this is missing, then all data passed in the
