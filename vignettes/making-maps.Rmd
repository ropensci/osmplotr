...
### 2.3 Downloading with `osm_structures` and `make_osm_map`
...
```
To understand why it may not work, it is useful to examine `connect_highways` in
more detail.
...
png (height=mapht, width=mapwd, file="map4.png")
print (map)
graphics.off ()
```
![map4](map4.png)
...
```{r map7}
bbox_small <- get_bbox (c (-0.118, 51.504, -0.110, 51.507))
map <- plot_osm_basemap (bbox=bbox_small, bg="gray95")
...
```
```{r, eval=FALSE}
dev.new (width=8, height=8 * diff (bbox_small [2,]) / diff (bbox_small [1,]))
print (map)
```
...
As indicated above (**2.3 Downloading with `osm_structures` and
...
```{r, eval=FALSE}
structures <- c ('highway', 'building', 'park', 'grass', 'tree')   
structs <- osm_structures (structures=structures, col_scheme='light')   
dat <- make_osm_map (structures=structs, bbox=bbox)
```
```{r, eval=FALSE}
# TODO: Check this!!
dat <- make_osm_map (structures=structs, bbox=bbox)
...
mapht <- mapwd * diff (bbox [2,]) / diff (bbox [1,])
png (height=mapht, width=mapwd, file="map8.png")
print (map)
graphics.off ()
```
![map8](map8.png)
...
download, gives:
```{r, eval=FALSE}
dat <- make_osm_map (osm_data=dat$osm_data, structures=structures, bbox=bbox)
```
```{r map9, echo=FALSE}
...
Objects in maps are overlaid on the plot accoording to the order of rows in
...
As described above (**2.3 Downloading with `osm_structures` and
