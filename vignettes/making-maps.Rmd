...
length (dat_T)
```
... 6,215 building polygons, 4,524 highway lines, and 3,182 trees.
...
bbox <- get_bbox (c (-0.14, 51.505, -0.11, 51.52))
...
```{r}
bbox <- get_bbox (c (-0.15, 51.5, -0.1, 51.52)) # zoom out again
...
plot_osm_basemap (bbox=bbox, bg="gray20", file="map19.png")
...
```{r highways2polygon, fig.width=7, message=FALSE, eval=TRUE}
highways <- c ("Kingsway", "Holborn", "Farringdon.St", "Strand",
               "Fleet.St", "Aldwych")
