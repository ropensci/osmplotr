...
`data.frame` of OSM structures, associated `key-value` pairs, unique suffices
which may be appended to data structures for storage purposes, and suggested
colours.
...
The suffices will include as many letters as are necessary to represent all
unique structure names.  A list of desired structures can also be passed to this
...
`osmplotr` includes one data object, `london`, which was generated as
follows.
```{r}
...
values, requiring the suffices for negated forms to be specified manually.  This
`structs` data frame was then used to download the `london` data with the
following code. 
```{r, eval=FALSE}
london <- list ()
