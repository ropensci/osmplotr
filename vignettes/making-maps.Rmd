...
png (height=500, width=500, file="map.png")
print (map)
graphics.off ()
```
...
in which call all matching that key will be returned regardless of `value`, or a
`key-value` pair. This is particularly useful for contrasting different kinds of
object, for example primary and non-primary highways:
...
The additional `key-value` pairs demonstrated above can be used to produce very
specific highlights.
```{r, eval=FALSE}
extra_pairs <- c ("name", "Royal.Festival.Hall")
