...
Package checks on CRAN and the various continuous integration services don't play nicely with OSM queries, so 
most of this vignette is disabled.

# Greater Melbourne.

This was the task that first prompted the work. It performs large queries to OSM, and the timeout has 
been reset.
```{r GMFuncs, echo=FALSE, message=FALSE}
## Set this to true to fetch from OSM servers. FALSE for cran or TravisCI checks
FetchOSM=FALSE
library(osmplotr)
library(osmdata)
...
```
Note that the coastline has several types of structure. We need to work on the lines.
```{r, eval=FetchOSM}
if (MapFetched) {
