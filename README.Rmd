...
[overpass api](http://overpass-api.de/). Maps can be made with the following
easy steps:
...
schemes. `cols` defines colours for each group (with only one here), while `bg`
...
separated regions.  Calling `add_osm_groups` with no `bg` argument forces all
points lying outside those defined groups to be allocated to the nearest groups,
and thus produces an inclusive grouping extending across an entire region.
...
```
And then passing the SpatialPolygon returned by `connect_highways` to
