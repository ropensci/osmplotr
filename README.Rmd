...
from `osmplotr` with no further modification. But first the easy steps to map
making:
...
fwd <- 600
fht <- fwd * diff (bbox [2,]) / diff (bbox [1,])
png (width=fwd, height=fht, filename="map2.png")
print (map)
graphics.off ()
```
...
png (width=fwd, height=fht, filename="map3.png")
print (map)
graphics.off ()
```
![](./figure/map3.png)
...
png (width=fwd, height=fht, filename="map4.png")
print (map)
graphics.off ()
```
![](./figure/map4.png)
...
png (width=fwd, height=fht, filename="map5.png")
print (map)
graphics.off ()
```
![](./figure/map5.png)
...
png (width=fwd, height=fht, filename="map6.png")
print (map)
graphics.off ()
```
![](./figure/map6.png)
...
png (width=fwd, height=fht, filename="map7.png")
print (map)
graphics.off ()
```
...
fwd <- 800
fht <- fwd * diff (bbox [2,]) / diff (bbox [1,])
png (width=fwd, height=fht, filename="map1.png")
...
map2 <- map + ggplot2::geom_text (dat=dat, mapping=aes, size=45, colour="black",
                                 family="Purisa", fontface=2,
                                 nudge_y=0.0005, nudge_x=0.0005)
map2 <- map2 + ggplot2::geom_text (dat=dat, mapping=aes, size=45, colour="white",
