...
Finally, `osmplotr` contains a function `add_osm_surface` to spatially interpolate a given set of spatial data points and colour OSM objects according to a specified colour gradient. This is illustrated here with a surface laid on regular points interior to `bbox`.
...
Passing the `bg` argument to `add_osm_surface` plots all objects beyond the convex hull of the `xy` points as that colour. (Such points may be included in the interpolation with `bg=NULL`.)
